...
public abstract class Area implements Playable {
    ...
    private void updateCamera () {
        ...
        if (viewCandidate != null) {
        	...
        	float wh =getCameraScaleFactor()/(float)2;
        	//We clamp the camera position between the borders, considering the camera's width and height, to avoid seeing black background
            viewCenter = clampPosition(viewCandidate.getPosition(),wh,getWidth()-wh,wh,getHeight()-wh);
    ...
    /**
     * Clamps the vector between the specified values
     * @param position (Vector): Original vector
     ...
     * @return (boolean) : The clamped version of the original vector
     */
    //TODO: MOVE TO MATH.VECTOR
    private Vector clampPosition(Vector position, float xMin , float xMax, float yMin,float yMax) {
    	float x=0;
    	x= Math.max(xMin, Math.min(xMax, (position.x)));
    	float y=0;
    	y= Math.max(yMin, Math.min(yMax, (position.y)));
